...
export default function CustomerPaymentPage() {
  ...
  try {
    const response = await fetch(
      `/api/sales/by-customer/${params.id}`,
      {
        method: 'GET',
        credentials: 'include',
      }
  ...
  const fetchPaymentHistory = async () => {
    ...
    try {
      const response = await fetch(
        `/api/payment-history?saleId=${selectedSale.id}`,
        { credentials: "include" },
      );
      const data = await response.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (amount <= 0 || amount > selectedSale.dueAmount) {
        throw new Error(
          `Amount must be between 0 and ${selectedSale.dueAmount}`,
        );
      }
      ...
      await Promise.all([
      fetchPaymentHistory(),
      fetchCustomer(),      // customer balance update
      fetchUnpaidSales(),   // list update
    ]);
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-slate-600 dark:text-slate-400 mt-1">
          Record payment for customer invoices
        </p>
      </div>
      ...
      <div className="card mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p
              className={`text-lg font-bold ${customer.balance > 0 ? "text-rose-600 dark:text-rose-400" : "text-slate-900 dark:text-white"}`}
            >
              Rs.{" "}
            ...
            <div className="card">
              ...
              <div className="space-y-3">
                {paymentHistory.map((payment) => (
                  <div
                    key={payment.id}
                    className="border-l-4 border-emerald-500 dark:border-emerald-400 pl-4 py-2"
                  >
